...
class AffordancePhiM(BaseModel):
    ...
    def predict_mask(
        ...
        for i in range(bs):
            ...
            # 在原有特征基础上引入太多噪声
            # pred_tokens_aligned = self.token_dim_reducer(pred_tokens)
            # point_embedding_enhanced, _ = self.point_token_cross_attn(
            #     query=point_embedding_, 
            #     key=pred_tokens_aligned, 
            #     value=pred_tokens_aligned
            # )
            # point_embedding_enhanced = point_embedding_ + point_embedding_enhanced
            # if self.point_enhanced:
            #     point_embedding_enhanced = self.FeatureInjector(
            #         point_emb=point_embedding_,
            #         raw_tokens=pred_tokens
            #     )
            # else:
            #     point_embedding_enhanced = point_embedding_
            point_embedding_enhanced = self.FeatureInjector(
                    point_emb=point_embedding_,
                    raw_tokens=pred_tokens
                )
