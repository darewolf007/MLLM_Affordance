...
@registry.register_model("aff_qwen3")
class AffordanceQwen3(BaseModel):
    PRETRAINED_MODEL_CONFIG_DICT = {
        "qwen3": "/data1/user/zhangshaolong/3D_ADLLM/configs/models/point_qwen.yaml",
    }
    ...
    def generate(
        ...
        with self.maybe_autocast(self.mix_precision):
            ...
            pred_masks = []
            if "points" in samples:
                output_ids = outputs.sequences[:, 1:] # output_ids = outputs.sequences[:, 1:-1]
                last_hidden_states = [
