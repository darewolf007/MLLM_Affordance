...
# import sys
# sys.path.append("/data1/user/zhangshaolong/3D_ADLLM")
# from models.uni3d.Uni3D.models.uni3d import create_uni3d
from gorilla.config import Config
...
@registry.register_model("aff_phi_uni3d")
class AffordancePhiUni3d(BaseModel):
    PRETRAINED_MODEL_CONFIG_DICT = {
        "phi_uni3D": "/workspace/project/Research_3D_Aff/Programme_affllm_code_chs/configs/models/point_phi_uni3D.yaml",
    }
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.point_encoder = create_uni3d(self.point_model_config)
        uni3d_checkpoint = torch.load(
            self.point_model_config.ckpt_path, map_location="cpu"
        )
        print("Load Checkpoint {}".format(self.point_model_config.ckpt_path))
        sd = uni3d_checkpoint["module"]
        self.point_encoder.load_state_dict(sd)
        self.point_encoder = self.point_encoder.to("cuda")
        # The PointBackbone
    ...
    def encode_point(self, points):
        with self.maybe_autocast(self.mix_precision):
            ...
            points_feat = self.point_encoder.encode_pc(points2bs).unsqueeze(1)
    ...
    def predict_mask(
        ...
        points2bs_color = torch.stack(points)
        points2bs = points2bs_color[:, :, :3]
    ...
    def prepare_response(self, answer):
