...
#     multi = True
#     if multi:
#         cfg_path = "3D_ADLLM/configs/datasets/affordance_datasets_multi.yaml"
#     else:
#         cfg_path = "3D_ADLLM/configs/datasets/affordance_datasets.yaml"
...
@registry.register_builder("affordance")
def build_affordance_dataset(name, cfg_path):
    if isinstance(cfg_path, str):
        cfg_path = [cfg_path]
    dataset = PointDataset(
        ann_paths=[path.join(registry.root, p) for p in cfg_path]
    )
    return dataset

@registry.register_builder("shapeomni")
def build_affordance_dataset(name, cfg_path):
    if isinstance(cfg_path, str):
        cfg_path = [cfg_path]
    dataset = PointImageVoxelDataset(
        ann_paths=[path.join(registry.root, p) for p in cfg_path]
    )
    return dataset
